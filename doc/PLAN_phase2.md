...
model MerchantStaff {
  id           String     @id @default(cuid())
  merchantId   String
  phone        String     @unique   // 子账号登录手机号
  passwordHash String               // 子账号登录密码
  displayName  String?              // 员工昵称
...
> **架构**：主账号通过 `User.merchantId` 直接关联店铺，无需 MerchantStaff 记录。子账号独立存在 MerchantStaff 表中，不关联 User 表。主账号密码存在 `User.passwordHash`，子账号密码存在 `MerchantStaff.passwordHash`。
...
- 提交后创建 `Merchant` 记录（status=PENDING），若用户已登录则设置 `User.merchantId` 关联店铺，并分配 MERCHANT 角色
...
- 功能：店主可添加员工（输入手机号，系统直接在 `MerchantStaff` 表创建子账号，默认密码 123456）

- 限制：子账号仅用于商家端登录，不在 User 表中创建记录。仅主账号可管理员工

#### 2.4 商家登录调整

- 审核通过的商家主账号登录后，`session.merchantId` 从 `User.merchantId` 获取

- 子账号登录后，`session.merchantId` 从 `MerchantStaff.merchantId` 获取
...
| `product:manage` | 商品管理（商家） | MERCHANT |
| `staff:manage` | 员工管理（商家主账号） | MERCHANT |
...
| 商家员工权限细分 | 首版不做，子账号共用 MERCHANT 角色。仅主账号（User.merchantId 非空）可管理员工 |
...
- `auth.service.ts` — 商家登录支持主账号（查 User.merchantId）和子账号（查 MerchantStaff）双路径，注入 `merchantId` 到 Session
...
> **2026-06-06 重构**：以下为 6月4日原始实施记录。6月6日进行了重大重构——删除 `BuyerProfile` 表、`MerchantStaff` 改为纯子账号表、主账号通过 `User.merchantId` 定位店铺、角色 `MERCHANT_STAFF` 重命名为 `MERCHANT`。详见 6月6日记录。

<details>
<summary>原始记录（已被后续重构覆盖）</summary>
...
### 2026-06-06 — 删除 BuyerProfile + 身份模型重构

### 核心变更

1. **删除 `BuyerProfile` 表**：买家密码和昵称统一到 `User.passwordHash` / `User.displayName`
2. **MerchantStaff 改为纯子账号表**：删除 `userId`、`isOwner`，新增 `phone`、`passwordHash`、`status`
3. **主账号直接关联店铺**：`User` 新增 `merchantId` 字段，主账号不再需要 MerchantStaff 记录
4. **角色重命名**：`MERCHANT_STAFF` → `MERCHANT`

### 新身份模型

| | User 表 | MerchantStaff 表 | 登录查哪 |
|---|---|---|---|
| **主账号** | 有，MERCHANT 角色，merchantId 指向店铺 | 无 | User 表 → `User.merchantId` 定位店铺 |
| **子账号** | 无 | 有，自带 phone + passwordHash | MerchantStaff 表 → `staff.merchantId` 定位店铺 |
| **买家** | 有，BUYER 角色 | 无 | User 表 |

### 密码存储

| 端 | 密码存储位置 |
| --- | --- |
| 买家 | `User.passwordHash` |
| 商家主账号 | `User.passwordHash` |
| 商家子账号 | `MerchantStaff.passwordHash` |
...
| 文件 | 改动 |
|------|------|
| `prisma/schema.prisma` | 删除 `BuyerProfile`；User 加 `merchantId`；MerchantStaff 重构 |
| `src/server/services/auth.service.ts` | `loginAsMerchant` 双路径（主账号+子账号）；`loginWithPassword`/`registerUser`/`loginByPhone`/`changePassword` 移除 BuyerProfile |
| `src/server/services/merchant.service.ts` | `createApplication` 设 User.merchantId；`addStaff`/`findStaffs`/`removeStaff`/`findByUserId` 适配新模型 |
| `src/server/actions/merchant.staff.actions.ts` | `addStaffAction` 直接创建 MerchantStaff 子账号 |
| `src/lib/auth.ts` | `getSessionUser` MERCHANT 分支支持主账号（User表）和子账号（MerchantStaff表） |
| `src/lib/types.ts` | `LoginEnd` 改为 `BUYER \| MERCHANT \| PLATFORM_ADMIN` |
| 其他文件 | `proxy.ts`、`upload/route.ts`、`merchant/layout.tsx`、`seed.ts`、staff 页面适配 |

**验证结果**（2026-06-06）

- `npx tsc --noEmit` — 零错误
- `prisma db seed` — 通过
- 数据库仅保留 3 个测试账号：admin / merchant / buyer
