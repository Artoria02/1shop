...
- 设计并创建 Prisma Schema：
  - `User` — 手机号、邮箱、密码 hash、头像、状态、注册来源、merchantId。**无 kind 字段**。通过 RBAC 角色（BUYER / MERCHANT）区分身份
  - `PlatformAdmin` — 平台管理员独立认证表（邮箱、密码哈希、昵称、头像）
  - `MerchantStaff` — 商家子账号表（手机号、密码哈希、显示昵称、状态），主账号通过 `User.merchantId` 定位店铺
...
- **买家端**：手机号 + 验证码登录 / 账号密码登录，密码使用 bcrypt 加密，统一存在 `User.passwordHash`
- **商家端**：主账号通过 User 表登录，子账号通过 MerchantStaff 表登录
- **后台**：邮箱 + 密码登录（独立 PlatformAdmin 表），密码使用 bcrypt 加密
